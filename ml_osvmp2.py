...
class NN_MBE():
    def __init__(self):
        ...
        ##############################################################################
        self.max_memory = int(os.environ.get('max_memory', 2000)) #MB, default 2GB
        ...
        dir_pair_ene = os.environ.get("dir_pair_ene", "/home/ubuntu/Shiwei/ml_datasets/pair_energy/pene_split/%s"%self.int_type)
        dir_rhf_ene = os.environ.get("dir_rhf_ene", "/home/ubuntu/Shiwei/ml_datasets/rhf_energy")
        self.file_pair_ene = {} 
        self.file_rhf_ene = {}
        for isystem in self.systems_test:
        ...
        with open("train_record.txt", "a") as frecord:
            frecord.writelines(f"{self.output_dir}\n")
...
#read
    ...
    def predict(self, model, ptype):
        ...
        # self.data_types = ["train", "valid"] # Added to disable test process
        for idx_type, itype in enumerate(self.data_types):
            
            # print(ptype)
            # if ptype != 'offdiag_remote':
            #     break
